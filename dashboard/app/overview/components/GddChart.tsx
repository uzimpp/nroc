...
const STAGE_LINES = CORN_STAGES.filter(
  (s) => s.id !== "plant" && s.id !== "harvest",
).filter((s, i, arr) => arr.findIndex((x) => x.gdd === s.gdd) === i);

const GDD_BASE = 10;
const GDD_CEIL = 30;

// Modified GDD per USDA sweet corn spec: cap t_min at base, cap t_max at ceiling, then average.
function buildSensorGdd(
  ...
  for (const [date, temps] of Array.from(byDay.entries()).sort(([a], [b]) =>
    a.localeCompare(b),
  )) {
    const t_min = Math.max(Math.min(...temps), GDD_BASE);
    const t_max = Math.min(Math.max(...temps), GDD_CEIL);
    const daily = Math.max(0, (t_max + t_min) / 2 - GDD_BASE);
    cum += daily;
...
function buildChartData(
  ...
  for (const log of logs) {
    ...
    const gdd = log.growth_progress_in_gdd;
  ...
  return (
    <Card noPad className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="label-caps text-[--text-muted] text-[10px]">
          Modified GDD · Base 10 °C · Cap 30 °C
        </span>
      </div>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <ComposedChart
          data={data}
          margin={{ top: 8, right: 72, left: 0, bottom: 4 }}
        >
            ...
            content={({ active, payload, label }) => {
              ...
              return (
                <div className="bg-black border border-white/10 rounded-[--radius-md] px-4 py-3 shadow-[--shadow-lg] text-xs min-w-[160px]">
                  <p className="text-muted text-[10px] pb-2 mb-2 border-b border-white/10">
                    {label}
                  </p>
                  {payload.map((p) => (
                      <div
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white/80">
                            {p.name === "sensorGdd" ? "Sensor" : "Logged"}
                          </span>
                        </div>
                        <span className="font-semibold text-white">
                          {p.value !== null
                            ? `${Number(p.value).toFixed(1)} GDD`
                            : "—"}
                        </span>
                      </div>
                    ),
          ...
          <Legend
            ...
            formatter={(v) =>
              v === "sensorGdd" ? "Sensor (daily acc.)" : "Logged observation"
            }
          />
